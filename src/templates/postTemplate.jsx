...
import { Badge, Box, Image, AspectRatio, Heading } from '@chakra-ui/react';
...
export default function BlogPost({ data, pageContext }) {
  ...
  const { tags } = data.allWpPost.edges[0].node;
  ...
  return (
    <BaseLayout>
      ...
      <Fade damping={0.5} duration={500} cascade triggerOnce>
        <Box>
          <Heading as="h1" fontSize="4xl" lineHeight="1.1">
            {post.title}
          </Heading>
        </Box>
        <PostMeta authorData={authorData} date={postDate} />
      </Fade>
      <BaseBox as="article">
        ...
        <Box className="wysiwyg" padding={{ base: 6, md: 12 }} paddingY={{ base: 6 }}>
          <div dangerouslySetInnerHTML={{ __html: post.content }} />
        </Box>
        <Box m={{ base: 6, md: 12 }}>
          {tags?.nodes.map(tag => (
            <Box display="inline" marginRight="3" key={tag.slug}>
              <Link to={`../../tag/${tag.slug}`}>
                <Badge rounded="brandRadius.badge" colorScheme="cyan">
                  {`# ${tag.name}`}
                </Badge>
              </Link>
            </Box>
      ...
      <NextAndPreviousPost previousPostSlug={previousPostSlug} nextPostSlug={nextPostSlug} />
    </BaseLayout>
  );
}
