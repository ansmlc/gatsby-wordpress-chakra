...
function NextAndPreviousPost({ previousPostSlug, nextPostSlug }) {
  return (
    <Stack
    ...
    >
      <Link to={`../../post/${previousPostSlug}`}>
        <PrimaryButton arrowLeft>Previous post</PrimaryButton>
      </Link>
      <Link to={`../../post/${nextPostSlug}`}>
        <PrimaryButton arrowRight>Next post</PrimaryButton>
      </Link>
    </Stack>
  );
}
