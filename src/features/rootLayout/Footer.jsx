...
const Footer = function ({ data }) {
  ...
  allMenuItems?.menuItems?.nodes?.map(item =>
    item.url?.includes('category') || item.url?.includes('tag') ? false : pageMenuItems.push(item)
  );
  ...
  return (
    <Box mt={10} color={useColorModeValue('gray.700', 'gray.200')}>
      ...
      <Box as="footer" d="flex" left="0" right="0" zIndex="2" marginTop="2rem" boxShadow="xl">
        <Text mx="auto" my="1rem" align="center" fontSize="xs">
          © {new Date().getFullYear()}. Built with {` `}
          <a href="https://www.gatsbyjs.com">Gatsby</a> and{' '}
          <a href="https://www.wpgraphql.com/">WordPress.</a>
        </Text>
      </Box>
    </Box>
  );
